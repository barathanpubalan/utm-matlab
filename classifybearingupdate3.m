...
for i = 1:length(varNames)
    ...
    trainLength = floor(0.7 * totalLength);
    
    trainSignal = signal(1:trainLength); % First 70% for training
    testSignal = signal(trainLength+1:end); % Remaining 30% for testing
    
    % Create subfolders for the variable in train and test directories
    mkdir(fullfile(trainDir, varName));
    mkdir(fullfile(testDir, varName));
    
    % Process train signal to generate images
    bf2cwtscg(trainSignal, fb, varName, trainDir);
    
    % Save test signal as MAT file in the test directory
    save(fullfile(testDir, varName, 'test_data.mat'), 'testSignal');
end

% Function definition
function bf2cwtscg(bfdata, cwtfb, bftype, outputDir)
    % Segment parameters
    win_len = 5120; % Length of each segment
